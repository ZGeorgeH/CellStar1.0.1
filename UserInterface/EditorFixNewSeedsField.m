...
function EditorFixNewSeedsField()
    global csui;
    currFrame = csui.session.states.Editor.currentFrame;
    
    % Not really needed here, but to be sure...
    UILoadSegmentationIfNeeded(currFrame);
    
    if ~isfield(csui, 'segBuf') || ...
            (length(csui.segBuf) < currFrame) || ...
            ~isfield(csui.segBuf{currFrame}, 'newSeeds')
        csui.segBuf{currFrame}.newSeeds = {};
    end
end
